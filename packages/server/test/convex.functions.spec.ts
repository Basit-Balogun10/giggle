...
describe("localConvex functions", () => {
  ...
  it("creates and lists gigs", async () => {
    const g1 = await localConvex.mutation("gigs.create", {
      title: "G1",
      payout: 100,
      tags: ["design"],
    });
    const g2 = await localConvex.mutation("gigs.create", {
      title: "G2",
      payout: 200,
      tags: ["dev", "react"],
    });
    ...
    const designOnly = await localConvex.mutation("gigs.list", {
      tag: "design",
    });
  ...
  it("creates a charge and records ledger entry", async () => {
    const charge = await localConvex.mutation("claims.createCharge", {
      amount: 500,
      metadata: { gigId: "g1" },
    });
