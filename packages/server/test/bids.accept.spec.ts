...
describe("localConvex bids acceptance -> ledger/charge integration", () => {
  ...
  it("creates ledger and pending charge when accepting a bid", async () => {
    ...
    const bid = await localConvex.mutation("bids.create", {
      gigId: gig.id,
      amount: 500,
      userId: "bidder1",
    });
    ...
    const acceptedEntry = store.ledger.find(
      (l: any) => l.type === "bid_accepted" && l.bidId === bid.id
    );
    expect(acceptedEntry).toBeDefined();

    const holdEntry = store.ledger.find(
      (l: any) => l.type === "hold" && l.meta && l.meta.bidId === bid.id
    );
    expect(holdEntry).toBeDefined();

    // charges should have a pending charge for the bid
    const charge = store.charges.find(
      (c: any) => c.metadata && c.metadata.bidId === bid.id
    );
