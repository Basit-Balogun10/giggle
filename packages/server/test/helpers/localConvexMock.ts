import type { CreateGigDTO, Gig, Bid, CreateBidDTO } from '../../../../common/src/types';
...
export const localConvexMock: any = {
  async mutation(name: string, payload?: unknown): Promise<MutationResult> {
    ...
    if (name === 'gigs.list') {
      ...
      if (q.tag) out = out.filter((g) => Array.isArray((g as any).tags) && (g as any).tags.includes(q.tag));
      if (typeof q.minPayout === 'number') out = out.filter((g) => g.payout >= q.minPayout);
      if (typeof q.maxPayout === 'number') out = out.filter((g) => g.payout <= q.maxPayout);
      if (q.q) {
    ...
    if (name === 'bids.listByGig') {
      ...
      if (p.userId) return store.bids.filter((b) => b.gigId === p.gigId && b.bidderId === p.userId).slice().reverse();
      return [];
    }
    ...
    if (name === 'bids.accept') {
      ...
      bid.status = 'accepted';
      bid.updatedAt = nowIso();
      store.ledger.push({ id: `l_${Date.now()}`, type: 'bid_accepted', bidId: bid.id, amount: bid.amount, meta: {} });
      const charge = { reference: `ps_local_${Date.now()}`, amount: bid.amount, status: 'pending', metadata: { bidId: bid.id, gigId: bid.gigId } } as any;
    ...
    if (name === 'claims.createCharge') {
      ...
      const charge = { reference: `ps_local_${Date.now()}`, amount: amount ?? 0, status: 'pending', metadata: metadata ?? {} } as any;
      store.charges.push(charge);
