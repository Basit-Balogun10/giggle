...
describe("localConvex bids functions", () => {
  ...
  it("creates bids and enforces visibility and actions", async () => {
    const gig = await localConvex.mutation("gigs.create", {
      title: "Bidable",
      payout: 1000,
      authorId: "poster1",
    });
    ...
    const b1 = await localConvex.mutation("bids.create", {
      gigId: gig.id,
      amount: 500,
    ...
    const posterView = await localConvex.mutation("bids.listByGig", {
      gigId: gig.id,
      userId: "poster1",
    });
    ...
    const otherView = await localConvex.mutation("bids.listByGig", {
      gigId: gig.id,
      userId: "someoneElse",
    });
    ...
    // bidder sees own bid
    const bidderView = await localConvex.mutation("bids.listByGig", {
      gigId: gig.id,
      userId: "bidder1",
    });
    ...
    // bidder updates bid
    const updated = await localConvex.mutation("bids.update", {
      bidId: b1.id,
      amount: 550,
      userId: "bidder1",
    });
    ...
    // poster counters
    const countered = await localConvex.mutation("bids.counter", {
      bidId: b1.id,
      counterAmount: 600,
      userId: "poster1",
    });
    ...
    // poster accepts
    const accepted = await localConvex.mutation("bids.accept", {
      bidId: b1.id,
      userId: "poster1",
    });
    ...
    const ledgerEntry = store.ledger.find(
      (l: any) => l.type === "bid_accepted" && l.bidId === b1.id
    );
