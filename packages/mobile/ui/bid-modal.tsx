...
export default function BidModal({ visible, onClose, gigId, onSubmit }: Props) {
  ...
  const styles = {
    overlay: { flex: 1, justifyContent: 'center' as const, padding: tokens.spacing.md, backgroundColor: 'rgba(0,0,0,0.4)' },
    container: { backgroundColor: tokens.colors.surface, borderRadius: tokens.radii.md, padding: tokens.spacing.md },
    title: { fontSize: tokens.typography.headingSize, fontWeight: '600' as const, marginBottom: tokens.spacing.sm, color: tokens.colors.text },
    label: { marginBottom: 4, color: tokens.colors.muted },
    input: { borderWidth: 1, borderColor: tokens.colors.muted, padding: tokens.spacing.sm, marginBottom: tokens.spacing.sm, color: tokens.colors.text },
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} transparent>
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <TextInput value={message} onChangeText={setMessage} style={{ ...styles.input, marginBottom: tokens.spacing.lg }} multiline />
          ...
          <View style={styles.actions}>
            ...
            <Pressable onPress={submit} style={{ padding: 8 }} disabled={loading}>
              <Text style={{ color: tokens.colors.secondary }}>{loading ? 'Sending...' : 'Send bid'}</Text>
            </Pressable>
          </View>
        </View>
      </View>
