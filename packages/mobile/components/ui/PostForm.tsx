...
export default function PostForm({onDone}: Props) {
  ...
  const submit = async () => {
    ...
    try {
  const created = await createGigWithRetry({title: title.trim(), description: description.trim(), payout: 0})
  ...
  return (
    <View style={{padding: 16, backgroundColor: tokens.colors.background}}>
      <Text style={{color: tokens.colors.text, fontSize: 18, marginBottom: 8}}>Create a gig</Text>
      {error ? <Text style={{color: tokens.colors.accent, marginBottom: 8}}>{error}</Text> : null}
      <TextInput
        placeholder="Title"
        value={title}
