...
export async function createGigWithConvex(payload: CreateGigPayload) {
  try {
    ...
    const ConvexClientCtor =
      (convexReact as any).ConvexReactClient ||
      (convexReact as any).ConvexClient ||
      (convexReact as any).ConvexClientCtor;
...
export async function createGigWithRetry(
