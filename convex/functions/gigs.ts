...
export const list = query({
  args: {
    filter: v.optional(v.record(v.string(), v.union(v.string(), v.number(), v.null()))),
  },
  ...
  handler: async (ctx, args) => {
    ...
    let out = rows as any[];
    if (f.tag) {
      out = out.filter((g) => Array.isArray(g.tags) && g.tags.includes(String(f.tag)));
    }
    if (typeof f.minPayout === 'number') out = out.filter((g) => g.payout >= Number(f.minPayout));
    if (typeof f.maxPayout === 'number') out = out.filter((g) => g.payout <= Number(f.maxPayout));
    if (f.q) {
